...
def ConstrainedContFracClosure (A : AdmissibleClass) : Prop :=
  bridgeClosed A ∧ gateClosed A ∧ ClassicalSourceBoundaryCarried

def NativeClosureTheorem (A : AdmissibleClass) : Prop :=
  ConstrainedContFracClosure A

def ConditionalRouteClosure (A : AdmissibleClass) : Prop :=
  ConstrainedContFracClosure A ∧ ClassicalSourceBoundaryCarried

theorem constrained_cont_frac_closure (A : AdmissibleClass) :
    ConstrainedContFracClosure A := by
...
theorem native_closure_theorem_checked (A : AdmissibleClass) :
  ...
  exact constrained_cont_frac_closure A

theorem conditional_route_closure_checked (A : AdmissibleClass) :
    ConditionalRouteClosure A := by
  exact And.intro (constrained_cont_frac_closure A) classical_source_boundary_carried_checked
